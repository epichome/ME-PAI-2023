...
class SWAGInference(object):
    ...
    def __init__(
        ...
        swag_epochs: int = 20,
        swag_learning_rate: float = 0.0459,
        swag_update_freq: float = 0.3,
        ...
        self.swag_diagonal_means = self._create_weight_copy()
        self.swag_diagonal_vars = self._create_weight_copy()
    ...
    def initiate_D(self):
        ...
        for name, param in current_params.items():
            self.D_dict[name] = collections.deque()
        


    def update_swag(self, initiate = False) -> None:
        """
        ...
        for name, param in current_params.items():
            ...
            self.swag_diagonal_means[name] = (self.current_n*self.swag_diagonal_means[name] + param)/(self.current_n + 1) 
            self.swag_diagonal_vars[name] = (self.current_n*self.swag_diagonal_vars[name] + param**2 )/(self.current_n +1)
            

            
            # Full SWAG
            if self.inference_mode == InferenceMode.SWAG_FULL and (not initiate):
                # TODO(2): update full SWAG attributes for weight `name` using `current_params` and `param`

                if self.swag_n_cols == self.deviation_matrix_max_rank:
                    self.D_dict[name].popleft()

                new_col = param - self.swag_diagonal_means[name]                
                self.D_dict[name].append(new_col)
    ...
    def fit_swag(self, loader: torch.utils.data.DataLoader) -> None:
        ...
        self.network.train()
        #Initiates the swag weights before start of the training
        self.current_n += 1
        self.initiate_D()
        self.update_swag( initiate = True)
        with tqdm.trange(self.swag_epochs, desc="Running gradient descent for SWA") as pbar:
            ...
            for epoch in pbar:
                ...
                # Check Probably want to round the argument to the right of %, to make sure it's an integer
                    statement_epoch = counter % int(1/self.swag_update_freq) == 0
                    if statement_epoch:
                        self.current_n += 1
                        self.update_swag()
                        if self.swag_n_cols < self.deviation_matrix_max_rank:
    ...
    def calibrate(self, validation_data: torch.utils.data.Dataset) -> None:
        ...
        self._prediction_threshold =  7.1499 / 10.0
    ...
    def predict_probabilities_swag(self, loader: torch.utils.data.DataLoader) -> torch.Tensor:
        ...
        for _ in tqdm.trange(self.bma_samples, desc="Performing Bayesian model averaging"):
            ...
            for (batch_xs, ) in loader:
                # self._update_batchnorm() # Maybe here???
                predictions = self.network(batch_xs)
        ...
        bma_probabilities = torch.nn.functional.softmax(bma_probabilities, dim = 1)
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            ...
            current_mean = self.swag_diagonal_means[name]
            current_std = torch.sqrt(0.5*(self.swag_diagonal_vars[name] - self.swag_diagonal_means[name]**2))
            ...
            # Full SWAG part
            if self.inference_mode == InferenceMode.SWAG_FULL and self.swag_n_cols >= 2:
                # TODO(2): Sample parameter values for full SWAG
                # raise NotImplementedError("Sample parameter for full SWAG")
                z_2 = torch.randn(self.swag_n_cols)
                D = self.get_D(name) @ z_2
                current_lr = torch.reshape(D, param.shape)
                sampled_param += current_lr
        ...
        # Check if it's really this easy, or if it should be inside the for-loop or smth else
    def get_D(self, name):
        deq = self.D_dict[name]
        D = torch.stack([tensor.flatten() for tensor in deq], dim=1)
        return D*np.sqrt(1/ (2 * (self.swag_n_cols - 1)))
