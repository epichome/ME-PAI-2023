...
class Model(object):
    ...
    def __init__(self):
        ...
        self.model = GaussianProcessRegressor(kernel=self.kernel,random_state=1,normalize_y=True,n_restarts_optimizer=1,copy_X_train=False)
    ...
    def make_predictions(self, test_x_2D: np.ndarray, test_x_AREA: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        posterior_mean, posterior_std = self.model.predict(test_x_2D,return_std=True)

        # TODO: Use the GP posterior to form your predictions here
        predictions = posterior_mean
        predictions[test_x_AREA == 1] += posterior_std[test_x_AREA == 1]

        return predictions, posterior_mean, posterior_std
...
def main():
    ...
    model.make_predictions(test_x_2D, test_x_AREA)
