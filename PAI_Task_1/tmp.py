...
class Model(object):
    ...
    def __init__(self):
        ...
        nystroem_approximation = Nystroem(kernel=kernel, n_components=100)

        gp = GaussianProcessRegressor(kernel=kernel, n_restarts_optimizer=10)
        self.model = make_pipeline(StandardScaler(), nystroem_approximation, gp)
    ...
    def make_predictions(self, test_x_2D: np.ndarray, test_x_AREA: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        # Adjust the predictions using the asymmetric cost function
        predictions = np.zeros_like(gp_mean)
        
        for i in range(len(predictions)):
            if test_x_AREA[i]:  # Check if it's a candidate area
                if gp_mean[i] < 0:  # Apply the cost function
                    predictions[i] = gp_mean[i] - (COST_W_UNDERPREDICT * gp_std[i])
                else:
